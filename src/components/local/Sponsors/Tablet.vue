  ...
  <div class="py-12">
    <div class="w-[85%] mx-auto">
      <table class="table w-full">
        ...
        <TableList v-for="(list, index) in sponsors.sponsorsList" :list="list"
          :index="(index + 1) + (pagination.page - 1) * pagination.page_size" />
      </table>

      <div class="flex justify-between items-center">
        <div>
          <p class="font-rubik text-[#1D1D1F]">{{ sponsors.sponsorsCount }} tadan {{
          (pagination.page - 1) * pagination.page_size + 1 }}-{{
              pagination.page_size * pagination.page >
                sponsors.sponsorsCount ? sponsors.sponsorsCount : pagination.page_size * pagination.page
            }} ko‘rsatilmoqda</p>
        </div>
        <div class="flex space-x-6">
          ...
          <div class="flex space-x-4 font-rubik">
            <div @click="pagination.page = pagination.page - 1 > 0 ? pagination.page - 1 : pagination.page"
              class="centering h-8 w-8 rounded-md border border-[#DFE3E8] bg-[#fff]"
              :class="{ 'bg-[#DFE3E8]': pagination.page == 1 }"><img src="@/assets/images/left-arrow.svg" alt=""></div>
            ...
            <div class="pagination-common" :class="activeClass2">{{ (pagination.page != 1 && pagination.page != 2 &&
            pagination.page != lastPage && pagination.page != lastPage + 1) ? pagination.page : '...'}}</div>
            <div @click="pagination.page = lastPage" class="pagination-common" :class="activeClass(lastPage)">{{
              lastPage
            }}</div>
            <div @click="pagination.page = lastPage + 1" class="pagination-common" :class="activeClass(lastPage + 1)">{{
            lastPage+ 1 }}</div>
            <div @click="pagination.page = pagination.page < lastPage+1 ? pagination.page + 1 : pagination.page"
              class="centering bg-[#fff] h-8 w-8 rounded-md border border-[#DFE3E8]"
              :class="{ 'bg-[#DFE3E8]': pagination.page == lastPage + 1 }"><img src="@/assets/images/right-arrow.svg"
                alt=""></div>
          </div>
        </div>
      </div>
